...
- **Reused, not rebuilt:** `StayCard`, `SearchFilters` (the chip row), `Select` (sort), `EmptyState`, `Button`, and the Home `CardGridSkeleton`. A sticky toolbar (`top-16`, under the header) holds the preference pills + Adjust, Sort, and the tag chip row; the live count and reset sit above the grid.
