...
The **admin CMS foundation now exists** (Phase 13.1): BetterAuth email/password login, a seeded SUPER_ADMIN, middleware-protected `/admin` routes, and the dashboard shell. What it does **not** have yet is any business CRUD — the Stays/Bookings/Experiences/Guides/Media/Settings modules are disabled "Soon" placeholders in the sidebar. So the founder can log in, but can't yet operate the site; the module build-out is the next milestone. Hosting onboarding is still a **"coming soon"** page (a WhatsApp interest hand-off), not a live flow. A content gap surfaced building Experiences: the `Experience` model has no structured facets (category, duration, difficulty, season, highlights), so that listing's filters run on **derived themes** rather than stored data — a real `Experience.category` (and friends) is the clean next step if the founder wants curated filtering, and `experience-themes.ts` is shaped so that swap touches one function. (Travel guides, by contrast, have a real `category` relation, so their filters are stored-data.) The Contact form has **no backend** — it hands off to WhatsApp/email; a real endpoint (with moderation/rate-limiting) is a future add.
