...
- **Schema v1.1 cuts scope the Developer Handoff lists as in-MVP.** Confirm with the founder before building around it, because §22 of that document makes it the contract:
  - **No customer accounts.** There is still no *guest* auth: `/login`, `/signup`, `/account`, `/account/saved` (wishlist collections) have no data model, and the trip timeline sidesteps it — a guest returns via the `BookingRequest.reference` code rather than logging in. **The admin back-office is the exception:** Phase 13.1 added a BetterAuth `User`/`Session`/`Account`/`AuthVerification` set for *admin operators* (see "The Admin CMS foundation"), which is entirely separate from guests. Customer accounts and wishlists remain unbuilt on v1.1.
