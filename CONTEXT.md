...
- **The big save now covers the nested collections.** `stayFormSchema` and `admin-stay.service` grew arrays for **highlights** (`StayHighlight`), **rooms** (`Room` — the public "Where you'll sleep"), **nearby places** (`NearbyPlace`), and **experiences** (`StayExperience` → `Experience`), plus **owner bio**, per-image **caption**, and an editable **slug**. On save the service reconciles them in one transaction (highlights/rooms/nearby are delete-and-recreate by array order; experiences update/create the shared `Experience` and reconcile the links — a removed experience is _unlinked_, never deleted; orphaned room/nearby Storage objects are binned after commit). `getStayForAdmin` returns them all.
- **The editor is sectioned + tabbed.** `StayForm` gained the nested editors (`CollectionEditors.tsx`): the Highlights editor is a plain **tick-list of labels** (matching the public ✓ list), rooms/nearby/experiences are repeatable cards with reorder + a per-item image. The edit page wraps it in `StayEditorTabs` — **Content · Reviews · Availability**.
...
The **admin CMS is now operable for stays** (Phases 13.1–13.2): BetterAuth login + a seeded SUPER_ADMIN, and a **full Stay Management module** — create/view/edit/publish/unpublish/soft-delete with search, filters, sort, server-side pagination and Supabase image uploads (`/admin/stays`). The founder can onboard, edit and publish homestays end to end. Still disabled "Soon" placeholders: **Bookings, Experiences, Travel Guides, Media, Settings** — the remaining modules follow the same admin service → `/api/admin/*` → UI pattern the Stay module establishes. Hosting onboarding is still a **"coming soon"** page (a WhatsApp interest hand-off), not a live flow. A content gap surfaced building Experiences: the `Experience` model has no structured facets (category, duration, difficulty, season, highlights), so that listing's filters run on **derived themes** rather than stored data — a real `Experience.category` (and friends) is the clean next step if the founder wants curated filtering, and `experience-themes.ts` is shaped so that swap touches one function. (Travel guides, by contrast, have a real `category` relation, so their filters are stored-data.) The Contact form has **no backend** — it hands off to WhatsApp/email; a real endpoint (with moderation/rate-limiting) is a future add.
