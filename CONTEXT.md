...
| `BETTER_AUTH_SECRET`                            | **Secret.** Signs admin session cookies. Stable — rotating logs everyone out.       |
| `BETTER_AUTH_URL`                               | The app's own origin (`http://localhost:3000` in dev). Admin auth base URL.         |
| `ADMIN_EMAIL` / `ADMIN_PASSWORD` / `ADMIN_NAME` | Credentials for the seeded SUPER_ADMIN (`npm run seed:admin`). Not read at runtime. |
...
- **Schema v1.1 cuts scope the Developer Handoff lists as in-MVP.** Confirm with the founder before building around it, because §22 of that document makes it the contract:
  - **No customer accounts.** There is still no _guest_ auth: `/login`, `/signup`, `/account`, `/account/saved` (wishlist collections) have no data model, and the trip timeline sidesteps it — a guest returns via the `BookingRequest.reference` code rather than logging in. **The admin back-office is the exception:** Phase 13.1 added a BetterAuth `User`/`Session`/`Account`/`AuthVerification` set for _admin operators_ (see "The Admin CMS foundation"), which is entirely separate from guests. Customer accounts and wishlists remain unbuilt on v1.1.
...
### The static & trust pages (Phase 11)

The pages that complete the MVP and carry the brand's trust: **About** (`/about`), **Contact** (`/contact`), **Become a Host** (`/become-a-host`), **Privacy** (`/privacy`), **Terms** (`/terms`), and the branded **404** and **500**. Reusable sections live in `src/components/marketing/`; only the two that need contacts read data.

- **Mostly static Server Components, one data read.** About, Privacy and Terms take no data. Contact and Become a Host read contacts through `getSiteData` (the same shell helper) for the WhatsApp/email links — never hardcoded. Per-page `metadata` + canonical on all five public pages; the error pages rely on their **status codes** (404/500) to stay out of the index rather than metadata.
- **Reusable sections, no duplicated layouts.** `PageHero` (eyebrow + display heading + intro + optional CTA slot) opens About/Contact/Become a Host; `CtaBand` (bark panel) closes them; `Faq` is an accessible native-`<details>` accordion (no client JS, the same pattern as the stay page's "show all reviews"); `LegalDocument` renders both Privacy and Terms from `LegalSection[]` data (a masthead, a sticky anchored table of contents, then numbered sections with paragraphs / bulleted / numbered-clause lists); `ErrorState` is the shared visual behind both 404 and 500 (a ghosted status numeral + roofline icon badge), so the two error screens can't drift.
- **Become a Host is a "coming soon" page** (per the founder): the eyebrow reads "For hosts · Coming soon", the CTA is a WhatsApp **"Register your interest"** hand-off, and the benefits / "how joining will work" copy is framed as what's coming — no live onboarding flow to maintain yet.
- **The Contact form works without a contact endpoint.** `ContactForm` (the one client island here) doesn't POST — there is no contact API, and standing one up would be an unmoderated spam surface. Instead it composes what you type into a message and hands off to the channel we actually answer on: **WhatsApp**, with an email fallback. It's honest (the button says where it goes), validates name + message before it can send, and stores nothing. `whatsappLink` (the pure helper) builds the URL.
- **Legal copy is accurate to the real system, not boilerplate.** `src/lib/legal-content.ts` describes what the platform actually does — request-to-book over WhatsApp, a name and phone per booking, no accounts, no payment taken through the platform, governed by Indian law — rather than promising systems Stayze doesn't run. Both docs share one `LEGAL_UPDATED` date.
- **404 & 500.** `src/app/not-found.tsx` (Server Component) renders inside the root layout, so the header/footer stay; it returns a real **404 status** (verified — unlike the dynamic slug pages, which 200-in-dev under the force-dynamic layout) and offers Home + Explore. `src/app/error.tsx` is the required Client boundary: it catches runtime errors, `console.error`s the detail for us but **never shows a stack trace or digest**, and offers `reset()` (Try again) + Home.
- **Footer wiring.** Privacy and Terms were added to the footer bottom bar (About/Contact/Become a Host were already in the footer's "Stayze" column and the header nav). Verified every page at desktop, tablet and mobile, the working Contact and Become-a-Host CTAs, both legal layouts, and the custom 404 and 500 (triggered via a throwaway route, since removed).

### Production readiness (Phase 12)

A hardening pass over the finished MVP — no new features, no redesigns. What it added and verified against a real **`next build && next start`** (the whole app compiled clean; 39 routes):

- **`robots.txt` and `sitemap.xml`, generated** (`src/app/robots.ts`, `src/app/sitemap.ts`). Robots allows everything public and disallows `/api/`, `/book/`, `/trip/` (the last two are per-booking `noindex` pages). The sitemap lists the static routes plus every published stay, experience and guide from the service layer, and **degrades to the static routes in a `try/catch`** so a DB blip (or CI's placeholder credentials at build) never 500s it. Verified: 33 URLs (9 static + 3 stays + 15 experiences + 6 guides).
- **Structured data completed.** Home now publishes `Organization` **and** `WebSite` via a schema.org `@graph`; the three detail pages (stay / experience / guide) each add a `BreadcrumbList` alongside their existing `LodgingBusiness` / `TouristAttraction` / `Article`. `src/lib/seo.ts` holds `SITE_URL`, `absoluteUrl` and `breadcrumbJsonLd` so the origin is written once.
- **Metadata consistency.** The root layout now carries default `openGraph` (siteName, `en_IN` locale, a fallback plantation image) and `twitter` (`summary_large_image`) so every page shares one social-card shape and no page can ship imageless; pages still override title/description/url/images. `metadataBase` was already set, and every public page already had a unique title, description and canonical.
- **Accessibility.** A **skip-to-content** link (`sr-only`, visible on focus) now leads the `<body>` and targets `<main id="main-content">`. (Focus-visible rings, semantic landmarks, `aria-labelledby` sections, form labels and image `alt` were already in place from earlier phases.)
- **Analytics hook points, inert.** `src/lib/analytics.ts` (`track`/`pageview`, no-op forwarders that detect GA/GTM/Clarity at runtime) and `src/components/analytics/Analytics.tsx` (env-gated `next/script` loader, mounted in the layout). With no `NEXT_PUBLIC_GA_ID` / `_GTM_ID` / `_CLARITY_ID` set — the state today — **nothing is injected**, so there's no third-party script or cookie surface. Turning analytics on later is an env change, not a code change. Placeholders added to `.env.example`.
- **Consistency fixes.** Removed the only two `transition-all` uses (ExploreIntro step dots → scoped `transition-[width,background-color]`, honouring the transform/opacity-only rule) and two stray invalid `size-4.5` classes (→ `size-4`). Grep confirmed **no default Tailwind colours** anywhere (palette is the eight brand tokens only) and **no `console.log`** left in.
- **Security re-verified against the production build.** No `SERVICE_ROLE`/`DATABASE_URL`/`ADMIN_API_KEY` string appears in any public HTML; the owner DTO still carries no phone/email and the caretaker phone stays gated until CONFIRMED; all four public GET APIs return `200 { success: true }`. No `NEXT_PUBLIC_*` secret exists — the only public vars are the Supabase URL/anon key (public by design) and the optional analytics IDs (measurement IDs, not secrets).
- **Known limitation (documented, deliberate):** the root layout is `force-dynamic` because the footer reads live `SiteSetting` from the DB on every request, so **no page is statically cached** — a small TTFB/Lighthouse-perf cost traded for always-current contacts and a DB-free CI build. If perf ever needs it, the fix is to make the shell's contacts `revalidate`-cached (ISR) rather than force-dynamic; it's a one-file change in `layout.tsx`, deferred for the MVP.

## The Admin CMS foundation (Phase 13.1)

The internal admin console — **authentication, protected routes, the reusable admin layout and the dashboard shell**. It is the base every future admin module builds on; no business CRUD yet. Auth is **BetterAuth** over the existing Prisma/Postgres. Note the admin `User` is an _operator_, not a guest — the customer portal still has no accounts (guests return by booking reference).

- **Four new Prisma models, isolated from the business schema:** `User` (with a `role` field), `Session`, `Account`, `AuthVerification` — BetterAuth's required tables, added in migration `20260721191212_add_admin_auth`. The verification model is named `AuthVerification` (not `Verification`) because a business enum already owns that name; BetterAuth is pointed back at it with `verification: { modelName: "authVerification" }`. `role` defaults to `SUPER_ADMIN` — the only role for the MVP; `ADMIN`/`EDITOR` are future-ready string values. **No existing business model was touched.**
- **Auth wiring, split so the seed can reuse it.** `src/lib/auth-factory.ts` (`createAuth(prisma, opts)`) is a **factory with no `server-only` import**, so both the app singleton and the seed can build BetterAuth from one config. `src/lib/auth.ts` (`server-only`) is the app instance over `@/lib/prisma`; `src/lib/auth-client.ts` is the browser client; `src/app/api/auth/[...all]/route.ts` mounts the handler. **Public sign-up is disabled** (`disableSignUp`) — this is a single-seeded-admin tool; only the seed passes `enableSignUp: true`. `role` is an `input: false` additional field, so it can never be set from a request.
- **Two-layer route protection.** `src/middleware.ts` runs on the Edge: an _optimistic_ session-cookie check (`getSessionCookie`, no DB) redirects any `/admin/*` (except `/admin/login`) to login before a render; it also sets `x-pathname`. The **authority** is the server: `src/app/admin/(dashboard)/layout.tsx` calls `auth.api.getSession` (a real DB validation) on every request — no session → `redirect("/admin/login")`, a session without SUPER_ADMIN → the `Unauthorized` dead-end, only a real SUPER_ADMIN reaches the shell. Because it's a Server Component running before any child, the guard can't be bypassed on the client.
- **Sessions:** HTTP-only, `SameSite=Lax`, 7-day cookie (`__Secure-` + `Secure` in production), signed with `BETTER_AUTH_SECRET`, refreshed a day at a time. Password hashing is BetterAuth's (scrypt). Verified end to end against a production build: unauth `/admin` → 307 login, wrong password → 401, correct login → 200 + cookie, `/admin/login` while signed in → 307 dashboard, sign-up → disabled, logout → cookies cleared + session ends + `/admin` bounces to login.
- **The admin has its own chrome, not the public shell.** The root layout reads `x-pathname` and, for `/admin`, renders `children` bare (skipping Header/Footer and the `getSiteData` read); the public `PublicShell` wraps everything else. The admin frame lives in `src/components/admin/`: `AdminShell` (fixed bark sidebar on desktop, a slide-in drawer on mobile — transform/opacity only), `Sidebar` (live routes light in gold — Dashboard, Stays, Experiences, Media and Settings are live as of Phase 13.6; Bookings and Travel Guides remain disabled "Soon" placeholders; logout), `TopBar` (mobile menu, disabled notification placeholder, a profile popover), `LoginForm` (validation, show/hide, remember-me, loading, one-line errors), `StatCard`, `Unauthorized`. The `ui/Input` primitive gained an optional `trailing` slot (for the password toggle) — a small, additive, reusable enhancement.
- **Dashboard (`/admin`)** — a Server Component, now **wired to real data** (`admin-dashboard.service.ts`). Four live stat cards (Stays/Bookings/Experiences/Travel guides, each with a published/new sub-count; the two with a module behind them link to it, the two without stay unlinked because there is nowhere to go). A **Needs attention** list built from real signals — a placeholder or missing booking WhatsApp number, reviews awaiting moderation, new booking requests, stays and experiences still in draft — each linking to where it is fixed. **Recent activity** is _derived_, not recorded: there is no audit-log table, so it merges the timestamps the schema already keeps (`BookingRequest.createdAt`, `Stay.updatedAt`, `Experience.updatedAt`, `ContactMessage.createdAt`) into one list with relative times. `Review` and `TravelGuide` are absent from the feed because neither has a created/updated column to order by. Quick actions all point at live routes.
- **Seeding:** `npm run seed:admin` (`prisma/seed-admin.ts`) is **non-destructive and idempotent** — unlike `seed.ts` (which wipes everything), it inserts only the one admin (skipping if the email exists) through BetterAuth's real sign-up path, so the password verifies on login. It builds its own Prisma client (like `seed.ts`) to dodge the `server-only` chain that throws under tsx. Credentials come from `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`ADMIN_NAME`.

### The Stay Management module (Phase 13.2)

The first CMS module — full CRUD for homestays, the primary onboarding tool. Create, view, edit, publish/unpublish, soft-delete, plus search, filters, sort, server-side pagination, and Supabase image uploads. Follows the architecture exactly: Client → REST → service → Prisma; Server Components → service → Prisma; images → Supabase Storage, `{ bucket, path }` in Postgres.

- **Admin API, namespaced and guarded.** `src/app/api/admin/stays/*` (list/create · get/patch/delete · status) and `/api/admin/upload` — **separate from the public `/api/stays`** (published-only, no owner phone), so drafts and private fields can never leak through the public contract. Every handler calls `requireSuperAdmin()` (`src/lib/admin-guard.ts`) — a live-session + role check that returns 401/403 — so a request straight to the JSON (curl, a script) can't skip the UI's guard. Added a `ForbiddenError` (403) to `src/lib/api.ts`.
- **The service is the write side.** `src/services/admin-stay.service.ts` owns list (search across name/owner/type/area/address, status/type/price filters, five sorts, skip/take pagination — all in Postgres), full detail, create, update, `setStayStatus`, and `softDeleteStay`. Create generates the next `propertyCode` (P0xx) and a unique slug, upserts the Owner, and writes amenities + images in one transaction; update reconciles images (drop removed, upsert desired with fresh hero/order) and bins orphaned Storage objects **after** the row work commits, reusing the media-lifecycle rule. **Publishing is refused without a cover photo.**
- **Schema changes (three additive migrations).** `add_stay_management`: `StayStatus` gained `HIDDEN` (Draft/Published/Hidden per the brief; unpublish → HIDDEN), `Stay.deletedAt` for **soft delete** (recoverable; delete also flips status off PUBLISHED so the public site — which reads PUBLISHED only — drops it with no change to the public service), and `Stay.menuImage{Bucket,Path}` — **the food menu is stored as a photo** (per the founder), shown on the stay page. `add_stay_maps_url`: `Stay.mapsUrl` (validated Google Maps link). No model was removed or repurposed.
- **One shared validation contract.** `src/lib/stay-form.ts` is a **pure** Zod module (`stayFormSchema`) used by both the REST route and the client form (`react-hook-form` + `zodResolver`) — the service stays the authority, but the two never drift. It carries the schema-required fields the brief omits (`story`, `area`, `bathrooms`) because a stay without them wouldn't render.
- **The UI** (`src/app/admin/(dashboard)/stays/*`, `src/components/admin/stays/*`): a **listing** (Server Component — the table, search/filter/sort/paginate all read the URL and re-query on the server; `StaysToolbar` and `StayRowActions` are the only client islands), a **multi-section form** (`StayForm` — owner, property, location, story, amenities multi-select, photos, trust & publishing; `ImageField`/`GalleryField` upload straight to `/api/admin/upload` on pick, with preview/remove/reorder/set-cover; a 422's field issues map back onto inputs), a read-only **view** page, and a reusable `ConfirmDialog` for delete. `react-hook-form` + `@hookform/resolvers` were added; the `img`-preview components use plain `<img>` (admin previews, not public SEO).
- **Verified end to end against a production build** — API (unauth→401, invalid→422, create→201, list/search, publish-without-cover→blocked, upload→**real Supabase object reachable at its URL**, attach+publish→200, public `/api/stays` shows it, unpublish→public drops it, status/type filters, price sort, pagination, soft delete→gone) **and the real UI** (login → listing → new-stay form → fill + upload cover + publish → landed on the view page with the uploaded image and auto-assigned `P004`), on desktop and mobile. All test rows, owners and Storage objects were cleaned up afterwards — the DB is back to the 3 seeded stays.

#### The editor became a full CMS (Phase 13.2b)

The editor was expanded so **every section of the public Stay page is editable from admin** — the editor mirrors the public page's order, so it reads as a replica. **No schema change** (an explicit constraint): the work maps the public page onto existing columns/models. Anything that would have needed a new column (per-room price, "how to reach", meal-plan text, a manual rating override, hero video, a dedicated OG image) was left out on purpose.

- **The big save now covers the nested collections.** `stayFormSchema` and `admin-stay.service` grew arrays for **highlights** (`StayHighlight`), **rooms** (`Room` — the public "Where you'll sleep"), and **nearby places** (`NearbyPlace`), plus **owner bio**, per-image **caption**, and an editable **slug**. On save the service reconciles them in one transaction (highlights/rooms/nearby are delete-and-recreate by array order; orphaned room/nearby Storage objects are binned after commit). Experiences were **also** a nested editor here originally, but Phase 13.4 moved them to a central module — the stay now carries an `experienceIds` **tick-list** that only _links_ managed experiences (`syncExperienceLinks`), never creates or edits their content. `getStayForAdmin` returns them all.
- **The editor is sectioned + tabbed.** `StayForm` gained the nested editors (`CollectionEditors.tsx`): the Highlights editor is a plain **tick-list of labels** (matching the public ✓ list), rooms/nearby are repeatable cards with reorder + a per-item image, and experiences are a **tick-list** of managed experiences (Phase 13.4). The edit page wraps it in `StayEditorTabs` — **Content · Reviews · Availability**.
- **Reviews moderation** (`ReviewsModeration` + `/api/admin/reviews/[reviewId]`, `/api/admin/stays/[id]/reviews`): approve / hide / delete, **content never editable**. Every action goes through `review.service`, which recomputes the denormalised rating in-transaction, so the public star average stays true.
- **Availability calendar** (`AvailabilityCalendar` + `/api/admin/stays/[id]/availability`): a month grid to block/unblock dates and set price overrides. New service functions (`getAvailabilityForAdmin`/`setAvailability`/`clearAvailability`) — no schema change. `BOOKED` is never set from admin (that's the booking flow's), so a real booking can't be clobbered.
- **Preview before publish.** `/stays/[slug]?preview=1` renders a draft/hidden stay for a signed-in SUPER_ADMIN (a preview banner marks it), via `getStayBySlug(slug, { includeUnpublished: true })` behind a session check. The editor's "Preview" button opens it.
- **Because the public Stay page already renders these collections from the DB, edits reflect immediately with no public-page changes.** Verified end to end against a production build: created a stay carrying highlights/rooms/nearby/experiences (all round-tripped through `getStayForAdmin`), set availability (blocked dates + a ₹9999 override, read back), moderated a seeded review (toggle + rating recompute + restore), and rendered the CoffeeCharm editor's Content/Reviews/Availability tabs (real data, seeded weekend overrides on the calendar). Test data cleaned up. One small deferral: the amenities picker is a checkbox multi-select, not yet a _searchable_ one (~15 amenities today).

### The Experience Management module (Phase 13.4)

Experiences became a **first-class, centrally-managed entity** with their own CMS, and the stay editor stopped creating them stay-side: a stay now just **ticks** which managed experiences it offers. Same architecture as Stays (Client → REST → service → Prisma; Server Components → service → Prisma; cover → Supabase `experiences` bucket). **No schema change** (an explicit founder constraint): the module is built strictly on the existing `Experience` columns.

- **What "no schema change" costs.** The brief asked the experience to carry category, duration, difficulty, a 3-state status, a gallery model, and a Details block (best time, what to bring, included, not included, notes) — every one of those is a new column, so they were **left out on purpose**. The module runs on `title`, `slug`, `excerpt` (short description), `story` (full description), one cover image, `metaTitle`/`metaDescription`, and the `isPublished` boolean. So status is **Draft / Published** (no Hidden), the list table has **Cover · Name · Assigned stays · Status · Created · Actions** (no Category/Duration/Difficulty), and there's no gallery or Details block.
- **The service is the write side.** `src/services/admin-experience.service.ts`: list (search title/slug, status filter, three sorts, pagination — all in Postgres; `_count.stays` gives the "Assigned stays" column), full detail, create, update, `setExperiencePublished`, `deleteExperience`, plus `getStayOptions` (the assign multi-select) and `getExperienceOptions` (the stay tick-list). Create/update generate a unique slug and reconcile the `StayExperience` links (`syncStayLinks` — added links append after a stay's existing experiences, removed links unlink, existing links keep their per-stay `sortOrder`). **Delete is a _hard_ delete** — the model has no soft-delete column and adding one is a schema change; the links cascade away and the cover object is binned. `ExperienceAdminError` is registered in `api.ts` (→ 400).
- **Admin API, namespaced and guarded.** `src/app/api/admin/experiences/*` (list/create · get/patch/delete · status), `requireSuperAdmin()` on every handler — separate from the public `/api/experiences` (published-only). `/api/admin/upload` gained an `experience` kind → `experiences` bucket.
- **The UI** (`src/app/admin/(dashboard)/experiences/*`, `src/components/admin/experiences/*`): a **listing** (Server Component; `ExperiencesToolbar` + `ExperienceRowActions` are client islands), one **reusable `ExperienceForm`** for create + edit (Basic information · Media · Assign to stays · Status & SEO), a **searchable `AssignStaysField`** multi-select (removable chips + a filtered checkbox list, marking draft stays), and `Preview` (opens `/experiences/[slug]` once published). Actions: Save, Preview, Publish/Unpublish, Delete. The Experiences nav item is now **live**.
- **The stay side flipped to a tick-list.** `stayFormSchema.experiences` (an array of inline `{title, description, image}`) became `experienceIds: string[]`; `StayForm`'s Experiences section is a checkbox list of managed experiences (drafts marked, with a "Create one" link when empty); `admin-stay.service` reconciles the links by id (`syncExperienceLinks`) instead of creating `Experience` rows. The public Stay page is unchanged — it already reads the junction filtered by `isPublished`, so only published experiences ever show, and edits reflect immediately.
- **Verified end to end against a production build:** created an experience assigned to a real stay (as a draft), confirmed detail/search/status-filter, confirmed the **stay's `experienceIds` picked it up**, published → public `/experiences/[slug]` renders → unpublished → public 404s; then **saved a stay with a changed `experienceIds`** (dropped 3 of 5, re-added them) with no data loss, and deleted the test experience (it unlinked from the stay via cascade). All test data cleaned up — experiences back to the 15 seeded.

### The Media Library & Site Settings (Phase 13.6)

Two small modules that centralise what was previously invisible or code-only. **No schema change** (the founder's standing rule), which shapes both of them decisively.

- **The Media Library is _derived_, not stored.** Schema v1.1 has no media/asset table, so rather than invent one the library is computed on read by joining two sources: **what exists** (`listBucket` — a new recursive walk in `lib/storage.ts`, since Supabase's `.list()` is one folder at a time and returns folders as null-id entries) and **what uses it** (every media reference across the eight referencing models — `StayImage`, `Room`, `NearbyPlace`, `Owner`, `Stay.menuImage`, `Experience`, `TravelGuide`, `ReviewImage`). That join is what makes "Used by" and "Unused" trustworthy rather than guessed. **Add a model to `collectReferences()` the moment it gains a media column**, or its images will read as unused and be offered up for deletion.
- **A third state the brief didn't ask for: _Missing_.** A row that points at an object no longer in Storage is a broken image on the public site, and the join surfaces it for free. It has its own filter.
- **Deleting is safe by construction.** An unused file deletes freely; a file still in use is **refused** (400) unless `force`, and then every reference is **detached in one transaction before** the object is removed — so "delete anyway" can never leave a row pointing at a dead path. This works only because every media column in the schema is nullable or lives on a deletable child row. **Replace** uploads the new bytes at a fresh server-generated path, repoints every reference in a transaction, then bins the old object (rolling the new one back if the repoint fails).
- **What no-schema costs here.** `altText`/`caption` can only be persisted for **stay images** — `StayImage` is the only model carrying those columns — so the dialog shows the fields for stay images and states the reason for everything else. There is no per-file "uploaded by", no tags, and no folders. Uploads land **unused** until an editor attaches them, which is honest rather than a bug.
- **The private bucket is handled deliberately.** `reviews` holds guest photos, so those items preview through a **batched signed URL**, are marked Private, and have **no Copy-URL** action (`getPublicUrl` throws for private buckets by design). Uploading into `reviews` from the library is refused — guest photos belong to a review.
- **Site Settings is four fields, because that is what exists.** `SiteSetting` has `whatsappNumber`, `supportPhone`, `supportEmail`, `instagramUrl`. The brief's Brand / Homepage / Footer / SEO / Booking-message / Contact-page / General sections (~40 fields: logo, favicon, hero copy, currency, time zone, Facebook/YouTube/LinkedIn/X, …) each need a new column and are **not** built. Rather than pretend, the page carries a **"Managed in code, not here"** panel pointing at the files those values actually live in (`lib/nav.ts`, `lib/home.ts`, `lib/seo.ts`, `lib/legal-content.ts`). `whatsappNumber` is the load-bearing one — `booking.service` refuses to build an enquiry without it — so this page is how the seeded `910000000000` placeholder gets replaced. The row is an **upsert on a fixed id**, so there is never more than one.
- **Verified end to end against a production build:** the derived index found all **75 objects with 0 unused and 0 missing** (every object referenced, every reference resolving — across all eight sources); used-by labels resolve to real entities ("Stay cover · Kaapi Nest"); every filter, sort and search; upload → detected unused → safe delete → back to baseline; alt-text/caption edit on a stay image and the **refusal** on a non-stay image; deleting a used file **refused** without force; replace repointing to a new path; settings PATCH propagating to the public `GET /api/site`, with invalid email rejected (422). Settings were restored and all test objects removed — the library is back to 75/0/0. Unauthenticated `/admin/media` redirects (307) and `/api/admin/media` 401s.

## State

Scaffold, **Schema v1.1 migrated and live**, **seeded with development data**, **a working data layer**, **a complete REST API**, **the brand foundation**, **the application shell**, **the reusable component library**, and the customer-facing pages: **Home, Explore, Stay detail, the booking flow, the My Trip dashboard, the Experiences section, the Travel Guide section, and the static & trust pages** — About, Contact, Become a Host (coming soon), Privacy, Terms, and branded 404/500 (Phases 4–11), followed by a **production-readiness hardening pass** (Phase 12): generated `robots.txt`/`sitemap.xml`, completed structured data (`WebSite` + `BreadcrumbList`), default social metadata, a skip-to-content link, inert analytics hook points, and a clean `next build`. **The customer-facing MVP frontend is complete and production-ready.** On top of it, **the admin CMS foundation** (Phase 13.1): BetterAuth authentication, a seeded SUPER_ADMIN, middleware-protected `/admin` routes, the reusable admin layout and the dashboard shell — no business CRUD yet.
...
The **admin CMS is now operable for stays and experiences** (Phases 13.1–13.4): BetterAuth login + a seeded SUPER_ADMIN, a **full Stay Management module** — create/view/edit/publish/unpublish/soft-delete with search, filters, sort, server-side pagination and Supabase image uploads (`/admin/stays`) — and an **Experience Management module** (`/admin/experiences`): create/edit/publish/unpublish/delete, search/status/sort/pagination, a cover upload, and a searchable **assign-to-stays** multi-select. Experiences are managed once here and **ticked** onto stays (the stay editor no longer creates them). On top of those, Phase 13.6 added a **Media Library** (`/admin/media`) — a _derived_ index of every storage object joined to what references it, with upload, replace, safe delete of unused files, and alt-text/caption editing for stay images — and **Site Settings** (`/admin/settings`), where the booking WhatsApp number and support contacts are edited. The founder can onboard/edit/publish homestays and experiences end to end, clean up orphaned media, and set the number every enquiry goes to. Still disabled "Soon" placeholders: **Bookings and Travel Guides** — they follow the same admin service → `/api/admin/*` → UI pattern the Stay and Experience modules establish. Hosting onboarding is still a **"coming soon"** page (a WhatsApp interest hand-off), not a live flow. Two deliberate content gaps remain on `Experience`, both from the founder's **"no schema change"** rule: the public `/experiences` listing still filters on **derived themes** (`experience-themes.ts`) rather than a stored `category`, and the admin experience editor has **no structured facets** (category, duration, difficulty) or gallery/Details block — adding a real `Experience.category` (and friends) is the clean next step if the founder wants curated filtering, and the swap touches one function. (Travel guides, by contrast, have a real `category` relation.) The admin experience **delete is hard** (no soft-delete column, by the same rule). The Contact form has **no backend** — it hands off to WhatsApp/email; a real endpoint (with moderation/rate-limiting) is a future add.
